...
class Settings(BaseSettings):
    ...
    # После выходных за раз приходит два-три десятка — с меньшим числом
    # понедельничный разбор растягивается на дни.
    hh_max_new_per_poll: int = 25
    # Граница для самого первого опроса: не трогать отклики старше стольких
    # дней. 0 — брать все. Дальше границей служит время прошлого опроса,
    # поэтому выключенный на выходные компьютер откликов не теряет.
    hh_skip_older_than_days: int = 0
    # Насколько глубоко догонять после перерыва. Две недели отпуска бот
    # разберёт, полгода простоя поднимать в CRM смысла нет.
    hh_catch_up_max_days: int = 14
